...
if node['resque_tng']['redis']['ipaddress']
  ...
  pool_exec << if node['resque_tng']['bundled']
                 "#{File.basename(node['resque_tng']['pool_exec'])} "
               else
                 "#{node['resque_tng']['pool_exec']} "
               end
  ...
  if node['resque_tng']['use_bluepill']
    ...
    bluepill_service 'resque-pool' do
      ...
      subscribes :load, "template[#{File.join(node['bluepill']['conf_dir'], 'resque-pool.pill')}]", :immediately
    end
  else
    template '/etc/init.d/resque_pool' do
      source 'resque_pool.init.erb'
      variables(
        el: platform_family?('rhel'),
        exec: pool_exec
      )
      mode '755'
    end
